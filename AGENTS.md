...
| Facts across surfaces | `tools/check_facts.py` | A hero number typed differently on the case, the home page, the Work page, the showcase or llms.txt; a Work headline that miscounts its cards; a stat-strip number missing from the case's facts.json |
