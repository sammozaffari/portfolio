...
for it in items:
    ...
    path, _, q = it["src"].partition("?"); src = pathlib.Path(path) if pathlib.Path(path).is_absolute() else ROOT / path; out = pathlib.Path(it["out"]) if pathlib.Path(it["out"]).is_absolute() else ROOT / it["out"]; out.parent.mkdir(parents=True, exist_ok=True)
