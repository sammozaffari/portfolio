...
<section class="sc-sec"><div class="sc-wrap">
  ...
  <p class="sc-prose"><code>assets/product/tokens.dtcg.json</code> is the source, written in the W3C Design Tokens Community Group format. <code>tools/build_tokens.py</code> compiles it to the stylesheet the screens load, and the lint fails if the two drift. The values below are read from the compiled file at build time, so they are the values the screens use. There is one mode: the DTCG resolver module is still a preview draft that says not to implement it, so this system does not pretend to have modes it cannot build.</p>
